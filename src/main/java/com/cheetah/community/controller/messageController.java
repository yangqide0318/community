...
import com.cheetah.community.service.MessageService;
...
public class messageController implements CommunityConstant {
    ...
    public String getLetterList(@PathVariable("conversationId") String conversationId,Model model, Page page){
        ...
        //设置分页信息
        page.setLimit(5);
        page.setPath("/message/notice/list/"+conversationId);
    ...
    @RequestMapping(path = "/notices/list/{conversationId}",method = RequestMethod.GET)
    public String getNoticesList(@PathVariable("conversationId") String conversationId,Model model, Page page){
        User user=hostHolder.getUser();
        //设置分页信息
        page.setLimit(5);
        page.setPath("/message/notices/list/"+conversationId);
        page.setRows(messageService.findNoticeCount(user.getId(),conversationId));
        //私信列表
        List<Message> noticeList = messageService.findNotices(user.getId(),conversationId, page.getOffset(), page.getLimit());
    ...
    private List<Integer> getNoticeIds(List<Message> messageList){
        List<Integer> ids=new ArrayList<>();
        if(messageList!=null){
            for(Message message:messageList){
